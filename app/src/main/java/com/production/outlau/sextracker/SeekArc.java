...
public class SeekArc extends View {
    ...
    private boolean startingWheel = true;
    private int startingProgress;
    private boolean isBelowMinimum = false;
    private int sign = 0;
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        mStartPaint.setColor(arcColor); //TODO
        mStartPaint.setAntiAlias(true);
        mStartPaint.setStyle(Paint.Style.STROKE);
        mStartPaint.setStrokeWidth(mProgressWidth);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int arcStart = mAngleOffset + mRotation ;
        ...
        //if(startingWheel) {
        //	canvas.drawArc(mArcRect, arcStart, startingProgress, false, mStartPaint);
        //}
        if(mEnabled) {
            // Draw the thumb nail
    ...
    private void updateOnTouch(MotionEvent event) {
        ...
        setPressed(true);
        //TODO dont run gettouch function twice
        sign = (int)(getTouchDegrees(event.getX(), event.getY())[1]);
        mTouchAngle = getTouchDegrees(event.getX(), event.getY())[0];
    ...
    private double[] getTouchDegrees(float xPos, float yPos) {
        ...
        //TODO merge angle < 0 if statements
        if (angle < 0) {
            positive = false;
        }
        else{
        ...
        if (tempPositive != positive && angle < 50 && angle > -50){
            tempSign = positive? 1 : -1 ;
        ...
        angle -= mStartAngle;
        return new double[]{angle,tempSign};
    }
    ...
    public int getChangeSignState() { return  sign; }

    public void setPositive(int progress){
        if(progress > 180) {
            this.positive = false;
        }
        else {
    ...
    public int getStartProgress(){ return this.startingProgress; }

    public void isBelowMin(boolean below){
        this.isBelowMinimum = below;
    }

    //TODO
    public void setStartingProgress(boolean startingWheel, int startingProgress){
        this.startingWheel = startingWheel;
